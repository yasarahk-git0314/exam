...
- The other file (`SOURCECOLUMNS.txt`) contains pipe delimited data representing 2 things:
  - the order in which these columns should
be displayed when combined with the data in `SOURCEDATA.TXT`.
  - The name of the column


### The problem:
- The data must be loaded to a desired destination as a single file.
- This data needs to be represented in a single csv file with a column row that is displaying the columns in the correct order as specified in `SOURCECOLUMNS.txt`.
